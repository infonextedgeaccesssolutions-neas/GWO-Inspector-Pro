...
<head>
    ...
    <style>
        /* ========== ORIGINAL DARK DESIGN (unchanged) – kept for brevity, same as before ========== */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body { font-family: var(--font); background: var(--navy); color: var(--gray6); font-size: 14px; min-height: 100vh; }
        #login-screen { position: fixed; inset: 0; background: linear-gradient(135deg, var(--navy) 0%, var(--navy3) 60%, #1a1050 100%); display: flex; align-items: center; justify-content: center; z-index: 1000; }
        .login-box { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--r-xl); padding: 40px; width: 420px; backdrop-filter: blur(20px); }
        ...
        .demo-btns { display: flex; gap: 8px; margin-top: 20px; }
        ...
        /* Main App Layout (same as original) */
        #app-shell { display: flex; min-height: calc(100vh - 58px); opacity: 0; transition: 0.3s; }
        #app-shell.ready { opacity: 1; }
        #nav { background: var(--navy2); border-bottom: 1px solid rgba(255,255,255,0.06); padding: 0 18px; display: flex; align-items: center; gap: 10px; height: 58px; position: sticky; top: 0; z-index: 200; }
        ...
        #nav-tabs { display: flex; gap: 2px; background: rgba(255,255,255,0.04); border-radius: 8px; padding: 3px; margin-left: auto; margin-right: auto; }
        .ntab { background: transparent; border: none; color: var(--gray3); padding: 6px 14px; border-radius: 6px; cursor: pointer; font-size: 12px; font-weight: 500; display: flex; align-items: center; gap: 5px; }
        .ntab.active { background: var(--blue2); color: #fff; }
        #nav-right { display: flex; align-items: center; gap: 6px; }
        .user-pill { display: flex; align-items: center; gap: 7px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; padding: 4px 12px; }
        ...
        .nbtn { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: rgba(255,255,255,0.7); padding: 5px 11px; border-radius: 7px; cursor: pointer; font-size: 11px; transition: 0.15s; }
        ...
        #sidebar { width: 220px; background: var(--navy3); flex-shrink: 0; border-right: 1px solid rgba(255,255,255,0.05); padding: 20px 12px; }
        ...
        .si.active { background: rgba(25,118,210,0.18); border: 1px solid rgba(25,118,210,0.35); }
        ...
        .ic-top { display: flex; gap: 10px; padding: 11px 13px; align-items: flex-start; }
        ...
        .ai-assistant { position: fixed; bottom: 20px; right: 20px; width: 320px; background: var(--navy2); border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 8px 24px rgba(0,0,0,0.3); z-index: 100; backdrop-filter: blur(8px); }
        .ai-header { padding: 12px 16px; background: var(--blue2); border-radius: 16px 16px 0 0; display: flex; justify-content: space-between; cursor: pointer; font-weight: 700; }
        .ai-body { display: none; padding: 12px; background: rgba(10,25,41,0.95); border-radius: 0 0 16px 16px; }
        .ai-body.open { display: block; }
        .ai-messages { max-height: 300px; overflow-y: auto; margin-bottom: 12px; font-size: 12px; }
        ...
        .ai-send { background: var(--blue2); border: none; border-radius: 20px; padding: 0 12px; cursor: pointer; color: white; }
        .toast { position: fixed; bottom: 20px; left: 20px; background: #1E293B; padding: 8px 16px; border-radius: 40px; transform: translateY(60px); opacity: 0; transition: 0.2s; z-index: 200; }
...
<div class="ai-assistant">
    <div class="ai-header" onclick="toggleAI()">🤖 AI Assistant</div>
...
<script>
    ...
    const BASE_SECTIONS = [
        ...
        {num:"1",name:"Studs / Inserts",items:[{id:"10101",desc:"Check traces of rust on the surface of the studs",criteria:"Check visually 100% of studs for signs of oxide",ctrl:"STU-001"},{id:"10102",desc:"Check damages on the threads of the studs",criteria:"Check visually 100% of studs for signs of hits or deformations",ctrl:"STU-002"},{id:"10103",desc:"Check studs traceability",criteria:"Check if at least one stud has the batch number engraved.",ctrl:"STU-003"},{id:"10104",desc:"Check length of the studs",criteria:"Measure with caliper: distance must be per spec.",ctrl:"STU-004",hasMeasure:true,measureUnit:"mm"},{id:"10105",desc:"Check tightening torque of studs",criteria:"Check at least 4 studs turning manually. The studs cannot turn freely.",ctrl:"STU-005"},{id:"10106",desc:"Check surface of the inserts",criteria:"No rests of consumables, tapes or resin.",ctrl:"STU-006"},{id:"10107",desc:"Check zero-degree mark and red stud",criteria:"Zero-degree mark engraved and red stud identified.",ctrl:"STU-007"},{id:"10108",desc:"Check plate with the reference to the Zero-degree position",criteria:"Placed next to the zero-degree stud.",ctrl:"STU-008"},{id:"10109",desc:"Check rubber ring between studs and inserts",criteria:"100% correctly located.",ctrl:"STU-009"},{id:"10110",desc:"Check Inserts protection against rust",criteria:"Plastic caps present.",ctrl:"STU-010"}]},
        {num:"2",name:"Lightning Protection System",items:[{id:"20101",desc:"Check lightning card",criteria:"Placed and legible.",ctrl:"LPS-001"},{id:"20102",desc:"Check fixing of LPS cable",criteria:"Tense, no hanging.",ctrl:"LPS-002"},{id:"20103",desc:"Check terminals of lightning cables",criteria:"Crimping correct.",ctrl:"LPS-003"},{id:"20104",desc:"Check LPS plate",criteria:"Fixed and undamaged.",ctrl:"LPS-004"},{id:"20105",desc:"Measure terminals conductivity",criteria:"≤50 mΩ",hasMeasure:true,measureUnit:"mΩ",ctrl:"LPS-005"},{id:"20106",desc:"Measure conductivity plate",criteria:"≤5 mΩ",hasMeasure:true,measureUnit:"mΩ",ctrl:"LPS-006"},{id:"20107",desc:"Check tip receptor",criteria:"Smooth transition.",ctrl:"LPS-007"},{id:"20108",desc:"Check side receptors",criteria:"Correct protrusion & paint.",ctrl:"LPS-008"},{id:"20109",desc:"Check drainage hole",criteria:"Free from obstruction.",ctrl:"LPS-009"}]}
    ];

    let state = { cover: {}, results: {}, details: {}, photos: {}, sections: null, pendingEdits: [] };
    let currentUser = null;
    // Ensure all users have a status field (fix for old localStorage data)
    ...
    // Migrate: add status to any user missing it
    ...
    // ========== LOGIN WITH STATUS CHECK (FIXED) ==========
    function doLogin() {
        const u = document.getElementById('li-user').value.trim();
        const p = document.getElementById('li-pass').value;
        const user = users.find(x => x.username === u && x.password === p);
        if (!user) { document.getElementById('login-error').innerText = 'Invalid credentials'; return; }
        // Ensure status exists (safety)
        if (!user.status) user.status = 'approved';
        if (user.status !== 'approved') {
            document.getElementById('login-error').innerText = `Account ${user.status}. Contact admin.`;
            return;
        }
        currentUser = user;
    ...
    function requestEditApproval(sectionIdx, itemIdx, field, newValue, oldValue, itemId) {
        state.pendingEdits.push({ id: Date.now(), sectionIdx, itemIdx, field, newValue, oldValue, itemId, requester: currentUser.name, status: 'pending' });
        saveState();
        if (isAdmin()) buildAdminPanel();
    ...
    // ========== UI BUILDING (Inspection, Sidebar, etc.) ==========
    function buildSidebar() { const sidebar = document.getElementById('sidebar'); sidebar.innerHTML = state.sections.map((s,i)=>`<div class="si" onclick="jumpToSection(${i})"><span>📌 ${s.num}. ${s.name}</span><span class="si-badge" id="sb-${i}">—</span></div>`).join(''); updateBadges(); }
    ...
    function buildInspection() { const cont = document.getElementById('insp-content'); if(!cont) return; cont.innerHTML = ''; state.sections.forEach((sec, si) => { const secDiv = document.createElement('div'); secDiv.id = `sec-${si}`; secDiv.innerHTML = `<div style="background:#1E293B; padding:12px; border-radius:16px; margin-bottom:12px"><h3>Section ${sec.num}: ${sec.name}</h3></div><div id="items-${si}"></div>`; cont.appendChild(secDiv); const itemsDiv = document.getElementById(`items-${si}`); sec.items.forEach(it => { const res = state.results[it.id] || ''; const details = state.details[it.id] || {}; itemsDiv.innerHTML += `<div class="ic ${res}" id="ic-${it.id}"><div class="ic-top"><div><div class="ic-id">${it.id}</div><div style="font-size:9px; color:#64748B">${it.ctrl||''}</div></div><div class="ic-body"><div class="ic-desc-editable" onclick="editField('${it.id}','desc','${escapeHtml(it.desc)}')">${escapeHtml(it.desc)}</div><div class="ic-crit-editable" onclick="editField('${it.id}','criteria','${escapeHtml(it.criteria)}')">${escapeHtml(it.criteria)}</div></div><div class="rb-row"><button class="rb ok-btn ${res==='ok'?'act':''}" onclick="setResult('${it.id}','ok')">OK</button><button class="rb nok-btn ${res==='nok'?'act':''}" onclick="setResult('${it.id}','nok')">NOK</button><button class="rb ${res==='na'?'act':''}" style="border-color:#64748B;color:#64748B" onclick="setResult('${it.id}','na')">N/A</button></div></div><div class="ic-extra ${res?'show':''}" id="extra-${it.id}"><div class="xcol"><label>Measurement</label><input value="${details.measurement||''}" onchange="updateDetail('${it.id}','measurement',this.value)"></div><div class="xcol"><label>Severity (L/M/H)</label><select onchange="updateDetail('${it.id}','severity',this.value)"><option value="">—</option><option ${details.severity==='L'?'selected':''} value="L">L — Low (cosmetic, monitor)</option><option ${details.severity==='M'?'selected':''} value="M">M — Medium (repair within 3 months)</option><option ${details.severity==='H'?'selected':''} value="H">H — High (immediate repair)</option></select></div><div class="xcol wide"><label>Comment</label><textarea onchange="updateDetail('${it.id}','comment',this.value)">${details.comment||''}</textarea></div><div class="photo-strip" id="photostrip-${it.id}">${renderPhotos(it.id)}${canEdit()?`<div class="add-photo" onclick="document.getElementById('file-${it.id}').click()">+</div><input type="file" id="file-${it.id}" multiple accept="image/*" style="display:none" onchange="attachPhotos('${it.id}',this)">`:''}</div></div></div>`; }); }); updateBadges(); }
    function renderPhotos(id) { return (state.photos[id]||[]).map((p,i)=>`<img class="photo-thumb" src="${p.data}" onclick="window.open('${p.data}')"><button style="background:#EF4444; border:none; border-radius:20px; padding:0 6px; margin-left:-20px; margin-top:-8px;" onclick="deletePhoto('${id}',${i})">✕</button>`).join(''); }
    ...
    function refreshPhotoStrip(id) { const strip = document.getElementById(`photostrip-${id}`); if(strip) strip.innerHTML = renderPhotos(id)+(canEdit()?`<div class="add-photo" onclick="document.getElementById('file-${id}').click()">+</div>`:''); }
    ...
    function refreshDashboard() { let total=0,ok=0,nok=0; state.sections.forEach(sec=>sec.items.forEach(it=>{ total++; const r=state.results[it.id]; if(r==='ok') ok++; else if(r==='nok') nok++; })); document.getElementById('kpi-grid').innerHTML = `<div class="kpi-card"><div class="kpi-value">${total}</div><div>Total</div></div><div class="kpi-card"><div class="kpi-value" style="color:#2E7D32">${ok}</div><div>OK</div></div><div class="kpi-card"><div class="kpi-value" style="color:#C62828">${nok}</div><div>NOK</div></div><div class="kpi-card"><div class="kpi-value">${Math.round((ok+nok)/total*100)}%</div><div>Complete</div></div>`; if(chart) chart.destroy(); chart = new Chart(document.getElementById('dashboard-chart'), { type: 'bar', data: { labels: ['OK','NOK'], datasets: [{ data: [ok,nok], backgroundColor: ['#2E7D32','#C62828'] }] } }); const tbody = document.querySelector('#summary-table tbody'); tbody.innerHTML = state.sections.map(sec=>{ let okc=0,nokc=0; sec.items.forEach(it=>{if(state.results[it.id]==='ok') okc++; else if(state.results[it.id]==='nok') nokc++;}); return `<tr><td>${sec.name}</td><td style="font-family:monospace">${sec.items.length}</td><td style="color:#2E7D32">${okc}</td><td style="color:#C62828">${nokc}</td><td>${nokc>0?'⚠️ Issues':'✅ Pass'}</td></tr>`; }).join(''); const defectBody = document.querySelector('#defect-table tbody'); const defects=[]; state.sections.forEach(sec=>sec.items.forEach(it=>{ if(state.results[it.id]==='nok') defects.push({id:it.id,desc:it.desc,sev:state.details[it.id]?.severity||'-',comment:state.details[it.id]?.comment||''}); })); defectBody.innerHTML = defects.map(d=>`<tr><td style="font-family:monospace">${d.id}</td><td>${d.desc}</td><td>${d.sev}</td><td>${d.comment}</td></tr>`).join(''); }
    ...
    function toggleAI() { document.getElementById('ai-body').classList.toggle('open'); }
    function askAI() { const query = document.getElementById('ai-query').value.trim(); if(!query) return; const msgDiv = document.getElementById('ai-messages'); msgDiv.innerHTML += `<div class="ai-message user">${escapeHtml(query)}</div>`; setTimeout(()=>{ let reply = "AI: "; if(query.toLowerCase().includes('severity')) reply = "Severity: L (Low - cosmetic, monitor), M (Medium - repair within 3 months), H (High - immediate repair)."; else if(query.toLowerCase().includes('stud')) reply = "Check all studs for rust, thread damage, and ensure at least one stud has a batch number engraved."; else reply += "Refer to the editable criteria for each item."; msgDiv.innerHTML += `<div class="ai-message">${reply}</div>`; msgDiv.scrollTop = msgDiv.scrollHeight; }, 300); document.getElementById('ai-query').value = ''; }

    window.setResult = setResult; window.updateDetail = updateDetail; window.attachPhotos = attachPhotos; window.deletePhoto = deletePhoto; window.exportToExcel = exportToExcel; window.exportToPDF = exportToPDF; window.exportToWord = exportToWord; window.exportJSON = exportJSON; window.resetAllData = resetAllData; window.jumpToSection = jumpToSection; window.editField = editField; window.toast = toast; window.showPage = showPage; window.toggleAI = toggleAI; window.askAI = askAI;
    window.approveEdit = approveEdit; window.rejectEdit = rejectEdit; window.approveUser = approveUser; window.rejectUser = rejectUser; window.deleteUser = deleteUser; window.showAddUserForm = showAddUserForm; window.hideAddUserForm = hideAddUserForm; window.addPendingUser = addPendingUser;
    loadState();
</script>
</body>
</html>